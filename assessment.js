...
assessmentButton.addEventListener(//イベント検知設定を追加
  'click',//クリックイベント、
  () => { //無名関数　アロー関数、イベントを検知したら実行される
    const userName = userNameInput.value;//入力欄（input）のの値を取得
    ...
    resultDivision.innerText = '';//divタグを空文字で上書きすることで空っぽにしている

    const headerDivision = document.createElement('div');//divタグの作成
    headerDivision.setAttribute('class','card-header text-bg-primary') ;
    headerDivision.innerText='診断結果';//タグの内側のテキストを作成
    ...
    //resultDivisionにもBootstrap のスタイルを適用する
    ...
    const script = document.createElement('script');//scriptタグの作成
    script.setAttribute('src', 'https://platform.twitter.com/widgets.js');//src属性の追加
